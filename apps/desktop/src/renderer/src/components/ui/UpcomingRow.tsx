...
export function UpcomingRow({ date, color, name, kind, amount }: UpcomingRowProps) {
  return (
    <div className="flex items-center gap-2 text-[12px] py-2 border-t border-[var(--hair)] min-w-0">
      <span className="font-['IBM_Plex_Mono'] text-[10.5px] text-[var(--ink-4)] w-[34px] flex-none">{date}</span>
      ...
      <span className="font-['IBM_Plex_Mono'] text-[12px] font-medium text-[var(--ink)] flex-none ml-1">{amount}</span>
    </div>
  )
}
