...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", padding: "7px 0", borderBottom: "1px solid var(--hair-3)" }}>
...
function HistoryRow({ h, next, symbol }: { h: HistoryEntry; next: HistoryEntry | undefined; symbol: string }) {
  const delta = next ? h.value - next.value : null
  return (
    <div style={{ display: "flex", alignItems: "center", padding: "6px 0", borderBottom: "1px solid var(--hair-3)", gap: 8 }}>
      <span style={{ fontSize: 11, color: "var(--ink-4)", width: 34, flexShrink: 0 }}>{h.date}</span>
      <span style={{ fontSize: 12, color: "var(--ink-3)", flex: 1 }}>{h.label}</span>
      <span style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: 12, color: "var(--ink)" }}>{symbol}{fmt(h.value)}</span>
      {delta !== null ? (
        <span style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: 11, width: 60, textAlign: "right", color: delta >= 0 ? "var(--accent)" : "var(--red)" }}>
...
function ChartTooltip({ active, payload }: { active?: boolean; payload?: { value: number; payload: { date: string } }[] }) {
  ...
  return (
    <div style={{
      background: "white", border: "1px solid var(--hair-2)", borderRadius: 6,
      padding: "5px 10px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)",
    }}>
      <div style={{ fontSize: 10, color: "var(--ink-4)", marginBottom: 2 }}>{payload[0].payload.date}</div>
  ...
  return (
    <div style={{ padding: "20px 24px 112px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
        ...
        <div style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: 24, fontWeight: 700, color: isLiability ? "var(--red)" : "var(--ink)", whiteSpace: "nowrap" }}>
      ...
      {prevEntry && (
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <span style={{
            ...
            {lastChange >= 0 ? "+" : "−"}{symbol}{fmt(Math.abs(lastChange))}
          </span>
        </div>
      )}
      ...
      <ResponsiveContainer width="100%" height={100}>
        <AreaChart data={chartData} margin={{ top: 4, right: 2, bottom: 0, left: 2 }} style={{ outline: "none" }} tabIndex={-1}>
          <defs>
          ...
          <XAxis dataKey="date" tick={{ fontSize: 9, fill: "var(--ink-4)" }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
      ...
      {/* Account info */}
      <div style={{ marginBottom: 4 }}><SectionTitle>账户信息</SectionTitle></div>
      ...
      {/* History drawer */}
      <Drawer.Backdrop isOpen={showHistory} onOpenChange={(v) => { if (!v) setShowHistory(false) }}>
