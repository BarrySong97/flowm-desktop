...
import { Outlet, useRouterState } from "@mock/_shim/router"
...
// Interaction-only stub: the add/edit modal opens only on button press; the static
// mock keeps the header button inert and never renders the form.
function AddSubModal(_props: {
  open: boolean
  onClose: () => void
  onSave: (form: unknown) => void | Promise<void>
}) {
  return null
}
...
export function SubscriptionsPage() {
  ...
  const subNameById = useMemo(
    () => new Map<string, any>((subscriptionsQuery.data ?? []).map((sub) => [String(sub.id), sub])),
  ...
  return (
    ...
    >
      ...
      >
        {/* Subscription list — fixed header, scrollable list, pinned totals (now on the right) */}
        <div
          style={{
            width: 390,
            flexShrink: 0,
      ...
      <AddSubModal open={showAdd} onClose={() => setShowAdd(false)} onSave={() => {}} />
    </div>
  )
}
