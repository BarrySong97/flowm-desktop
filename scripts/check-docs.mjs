...
function writeStopHookOutput(output) {
...

{
  ...
  if (BASE && !/^[\w./@~^-]+$/.test(BASE)) {
    warn("Warning: --base contains invalid characters and was ignored.")
    BASE = null
  }
}
...
Options:
  ...
  --hook         Stop-hook mode. Emits only Codex Stop hook JSON.
...
if (existsSync(cfgPath)) {
  try {
    ...
    warn(`Warning: failed to parse check-docs.config.json: ${error.message}`)
  }
}
...
function gitChangedFiles(base) {
  ...
  if (base) {
    try {
      ...
      warn(`Warning: git diff ${base}...HEAD failed: ${error.message}`)
    }
  }
...
if (changed) {
  ...
  for (const changedFile of changed) {
    ...
    const prefix = prefixes.find(
      (candidate) => candidate === "" || changedFile.startsWith(candidate),
    )
