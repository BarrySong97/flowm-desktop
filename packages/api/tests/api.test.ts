...
function countRows(db: Database, table: string, where = "1 = 1") {
  const row = db.$client.prepare(`select count(*) as count from ${table} where ${where}`).get() as { count: number }
...
async function createGoldenFixture(api: FlowmApi) {
  ...
  const refund = expectOk(await api.createCashflowEvent({
  ...
  const mortgagePayment = expectOk(await api.createCashflowEvent({
  ...
  const ignored = expectOk(await api.createCashflowEvent({
  ...
  expectOk(await api.addAssetSnapshot({ assetItemId: bank.id, snapshotAt: "2026-06-01T00:00:00.000Z", valueAmount: "50000.00" }))

  const fund = expectOk(await api.createAssetItem({ name: "基金账户", assetType: "fund", valuationMethod: "manual_market_value" }))
  expectOk(await api.addAssetSnapshot({ assetItemId: fund.id, snapshotAt: "2026-05-01T00:00:00.000Z", valueAmount: "100000.00" }))
  expectOk(await api.addAssetSnapshot({ assetItemId: fund.id, snapshotAt: "2026-06-01T00:00:00.000Z", valueAmount: "120000.00" }))

  const realEstate = expectOk(await api.createAssetItem({ name: "房产", assetType: "real_estate", valuationMethod: "estimated_value" }))
  expectOk(await api.addAssetSnapshot({ assetItemId: realEstate.id, snapshotAt: "2026-06-01T00:00:00.000Z", valueAmount: "3000000.00" }))

  const mortgageLiability = expectOk(await api.createAssetItem({ name: "房贷负债", assetType: "liability" }))
  expectOk(await api.addAssetSnapshot({ assetItemId: mortgageLiability.id, snapshotAt: "2026-06-01T00:00:00.000Z", valueAmount: "1800000.00" }))

  const iCloud = expectOk(await api.createSubscription({
    name: "iCloud",
    merchant: "Apple",
    amount: "21.00",
  ...
  const mortgage = expectOk(await api.createLoan({
    name: "房贷",
    lender: "招商银行",
  ...
  const period = expectOk(await api.createBudgetPeriod({
    budgetSetId: budgetSet.id,
    periodKind: "monthly",
    periodStart: "2026-06-01",
    periodEnd: "2026-06-30",
  }))
  const foodBudget = expectOk(await api.createBudgetItem({
...
describe("@flowm/api — clean-slate data model", () => {
  it("initializes only the clean-slate schema and rejects negative core amounts", async () => {
    ...
    expect(() => db.$client.prepare(`insert into cashflow_events
        ...
        values ('good', '2026-06-01', '1.00', 'CNY', 'out', 'expense', 'manual', 1, 'active', 'manual', 'now', 'now')`).run()).not.toThrow()
  }, 30_000)
  ...
  it("keeps every cashflow kind queryable while default spend excludes non-expense activity", async () => {
    ...
    const all = expectOk(await api.listCashflowEvents({ dateFrom: "2026-06-01", dateTo: "2026-06-30", status: "active" }))
    expect(all.map((event) => event.flowKind)).toEqual(expect.arrayContaining([
      "income",
      "expense",
      "transfer",
      "refund",
    ...
    const everydaySpend = expectOk(await api.getCashflowSummary({
    ...
    const debtPayments = expectOk(await api.getCashflowSummary({
    ...
    const refunds = expectOk(await api.getCashflowSummary({
      metric: "refunds",
      dateFrom: "2026-06-01",
      dateTo: "2026-06-30",
    }))
  ...
  it("imports statement lines as evidence and only creates cashflow on explicit conversion", async () => {
    ...
    const result = expectOk(await api.importStatement({
      sourceName: "alipay",
    ...
    const converted = expectOk(await api.convertStatementLinesToCashflowEvents({ importId: result.batchId }))
  ...
  it("lists lightweight sparkline points grouped per asset with per-asset truncation", async () => {
    ...
    assetIds.forEach((id, index) => { if (!firstIndex.has(id)) firstIndex.set(id, index) })
  ...
  it("keeps subscriptions and loans as future pressure without creating cashflow or liability assets", async () => {
    ...
    const pressure = expectOk(await api.getFutureFixedPressure({
      dateFrom: "2026-06-01",
      dateTo: "2026-06-30",
    }))
  ...
  it("treats budgets as planned boundaries with optional cashflow reference progress", async () => {
    ...
    const fixture = await createGoldenFixture(api)
    const progress = expectOk(await api.getBudgetReferenceProgress({ budgetPeriodId: fixture.budget.period.id }))
  ...
  it("object links explain relationships without changing core aggregates", async () => {
    ...
    const spendBefore = expectOk(await api.getCashflowSummary({
      metric: "everyday_spend",
      dateFrom: "2026-06-01",
      dateTo: "2026-06-30",
    })).amount

    const occurrences = expectOk(await api.listSubscriptionOccurrences({ subscriptionId: fixture.future.iCloud.id }))
    expectOk(await api.createObjectLink({
    ...
    expect(expectOk(await api.getCashflowSummary({
      metric: "everyday_spend",
      dateFrom: "2026-06-01",
      dateTo: "2026-06-30",
  ...
  it("seeds and validates six months of deterministic demo data across the clean schema", async () => {
    ...
    expect(events.map((event) => event.flowKind)).toEqual(expect.arrayContaining([
      "income",
      "expense",
      "transfer",
      "asset_movement",
    ...
    const specialIncluded = db.$client.prepare(`select coalesce(sum(cast(amount as real)), 0) as total
        from cashflow_events
        where id like 'demo_%'
          and status = 'active'
          and include_in_analytics = 1
          and flow_kind in ('transfer', 'asset_movement', 'debt_payment', 'refund', 'adjustment')`).get() as { total: number }
    ...
    const futurePressure = expectOk(await api.getFutureFixedPressure({ dateFrom: "2026-06-13", dateTo: "2026-08-11" }))
  ...
  it("reseeds demo rows without deleting non-demo manual data", async () => {
    ...
    const manual = expectOk(await api.createCashflowEvent({
  ...
  it("surfaces missing FX instead of silently using today's rate", async () => {
    ...
    expectOk(await api.addAssetSnapshot({
      assetItemId: usd.id,
