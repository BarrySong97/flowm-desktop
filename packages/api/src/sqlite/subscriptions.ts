...
import { subscriptionOccurrences, subscriptions, type SubscriptionInsert, type SubscriptionRow } from "@flowm/db"
...
export abstract class SubscriptionsApi extends AssetsApi {
  async listSubscriptions(input: ListSubscriptionsInput = {}): Promise<Result<SubscriptionSummary[]>> {
    try {
      const rows = this.db
        .select()
        .from(subscriptions)
        .where(input.status ? eq(subscriptions.status, input.status as SubscriptionRow["status"]) : undefined)
  ...
  async createSubscription(input: CreateSubscriptionInput): Promise<Result<SubscriptionSummary>> {
    try {
      ...
      this.db
        ...
        .run()
      return ok(this.mapSubscription(this.db.select().from(subscriptions).where(eq(subscriptions.id, id)).get()!))
    } catch (error) {
      return fail(error)
    }
  }
  ...
  async updateSubscription(input: UpdateSubscriptionInput): Promise<Result<SubscriptionSummary>> {
    try {
      ...
      this.db.update(subscriptions).set(set).where(eq(subscriptions.id, toSqlId(input.id))).run()
      return ok(this.mapSubscription(this.db.select().from(subscriptions).where(eq(subscriptions.id, toSqlId(input.id))).get()!))
    } catch (error) {
      return fail(error)
    }
  }
  ...
  async archiveSubscription(input: { id: FlowmId }): Promise<Result<void>> {
    try {
      this.db.update(subscriptions).set({ status: "canceled", updatedAt: nowIso() }).where(eq(subscriptions.id, toSqlId(input.id))).run()
      return ok(undefined)
    } catch (error) {
      return fail(error)
    }
  ...
  async generateSubscriptionOccurrences(input: GenerateOccurrenceInput): Promise<Result<{ generated: number }>> {
    try {
      ...
      const subs = this.db.select().from(subscriptions).where(and(...conds)).all()
      let generated = 0
      for (const sub of subs) {
        ...
        while (due <= input.throughDate && safety++ < 500) {
          const exists = this.db
            ...
            .where(and(eq(subscriptionOccurrences.subscriptionId, sub.id), eq(subscriptionOccurrences.dueDate, due)))
            .get()
          if (!exists) {
            this.db
              .insert(subscriptionOccurrences)
              .values({ id: newId("subocc"), subscriptionId: sub.id, dueDate: due, amount: sub.amount, currency: sub.currency, createdAt: nowIso() })
              .run()
            generated++
          }
  ...
  async listSubscriptionOccurrences(input: ListSubscriptionOccurrencesInput = {}): Promise<Result<SubscriptionOccurrenceSummary[]>> {
    try {
      const conds: SQL[] = []
      if (input.subscriptionId) conds.push(eq(subscriptionOccurrences.subscriptionId, toSqlId(input.subscriptionId)))
