...
- Future subscription obligations live in `subscriptions` and are projected from plan rules at read
  time. Loan plans and their amortization forecasts live in `loans` and `loan_payment_occurrences`.
...
- Elapsed subscription projection dates are not actual deductions. Actual subscription history and
  totals come only from explicitly linked `cashflow_events`.
